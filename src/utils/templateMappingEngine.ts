...
export function getLayerBinding(layer: Layer, _allLayers: Layer[] = []): SupportedBinding | null {
  // 1. Photo and Signature MUST map ONLY to image or placeholder containers — NEVER replace text layers!
  if (layer.type === 'placeholder') {
    const p = layer as PlaceholderLayer;
    if (p.placeholderKey === 'photo' || p.placeholderType === 'photo' || p.label?.toLowerCase().includes('photo')) {
      return 'PHOTO';
    }
    if (p.placeholderKey === 'signature' || p.placeholderType === 'signature' || p.label?.toLowerCase().includes('signature')) {
      return 'SIGNATURE';
    }
  }
  ...
  // 5. Image or Placeholder Photo/Signature layers by ID / Name
  ...
  // 6. Text Layer Token & ID/Name Matching
  if (layer.type === 'text') {
    ...
    if (/(?:^|[_\s-])var_last_name|txt_last_name|surname_var|last_name_var/i.test(layerIdentifier)) return 'LAST_NAME';
    if (/(?:^|[_\s-])var_dob|txt_dob|dob_var/i.test(layerIdentifier)) return 'DOB';
    if (/(?:^|[_\s-])var_gender|txt_gender|var_sex|txt_sex|var_jinsi|txt_jinsi|gender_var/i.test(layerIdentifier)) return 'GENDER';
    if (/(?:^|[_\s-])var_nida|txt_nida|nida_var|barcode_nida/i.test(layerIdentifier)) return 'NIDA_NUMBER';

    // Check Composite Text Label Prefixes
    if (/^jina\s*[:：\-]/i.test(text) && !/jina\s+la\s+(?:mwisho|ukoo)/i.test(text)) return 'FIRST_NAME';
    if (/^(?:jina\s+la\s+(?:mwisho|ukoo)|surname|last\s*name)\s*[:：\-]/i.test(text)) return 'LAST_NAME';
    if (/^(?:middle\s*name|la\s+kati)\s*[:：\-]/i.test(text)) return 'MIDDLE_NAME';
    if (/^(?:tarehe\s+ya\s+kuzaliwa|date\s+of\s+birth|dob)\s*[:：\-]/i.test(text)) return 'DOB';
    if (/^(?:jinsi|jinsia|sex|gender)\s*[:：\-]/i.test(text)) return 'GENDER';
    if (/^(?:national\s*id\s*no|nambari\s*ya\s*nida|nida\s*no)\s*[:：\-]/i.test(text)) return 'NIDA_NUMBER';
  }
  ...
  if (layer.type === 'text') {
    ...
    switch (binding) {
      case 'FIRST_NAME': {
        ...
        } else if (/^jina\s*[:：\-]\s*/i.test(text)) {
          text = text.replace(/^jina\s*[:：\-]\s*.*/i, `JINA : ${val}`);
        } else {
          text = val;
        }
        break;
